...
import { logoutWithGoogle } from '../../../../../app/modules/Auth/__hooks__';
  ...
  const logoutClick = () => {
    if (auth.loginType === 'L') {
      ...
      firebase.auth().signOut().then(function () {
        setTimeout(() => {
          const execute = async () => {
            const response = await logoutWithGoogle(auth.accessToken, auth.data.email);
            if (response.success) {
              dispatch({ type: authTypes.LOGOUT_WITH_GOOGLE });
            } else {
              dispatch({ type: componentTypes.SHOW_SNACKBAR, snackBarStatus: 'error', snackBarDuration: 5000, snackBarMessage: response.message });
            }
          }
          execute();
        }, 1000)
        ...
        <div className="d-flex align-items-center mt-5">
          ...
          >
            <div className="symbol-label" style={{
              backgroundImage: `url(${auth.loginType === 'L' ? '/media/custom/avatar.png' : auth.data.photoURL === undefined ? null : auth.data.photoURL})`
            }} />
          ...
          <div className="d-flex flex-column">
            ...
            >
              {auth.loginType === 'L' ? auth.data.FirstName + ' ' + auth.data.LastName : auth.data.displayName === undefined ? null : auth.data.displayName}
            </a>
            <div className="text-muted mt-1">{auth.data.designation === undefined ? null : auth.designation}</div>
              ...
              <a href="#" className="navi-item">
                <span className="navi-link p-0 pb-2">
                  ...
                  <span className="navi-text text-muted text-hover-primary">
                    {auth.loginType === 'L' ? auth.data.EmailAddress : auth.data.email === undefined ? null : auth.data.email}
                  </span>
                </span>
              </a>
            </div>
            ...
            <button className="btn btn-light-primary btn-bold" onClick={logoutClick}>Logout</button>
          </div>
        </div>

        <div className="separator separator-dashed mt-8 mb-5" />
        <div className="navi navi-spacer-x-0 p-0">
          <NavLink to="/admin/user/profile" className="navi-item">
