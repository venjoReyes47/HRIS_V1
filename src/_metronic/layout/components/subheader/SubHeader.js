...
export function SubHeader() {
  ...
  const getSubHeaderTitle = () => {
    return routes.filter((element) => {
      if (element.layout + element.subLayout + element.path === location.pathname) {
        return element;
      } else {
        return null;
      }
  ...
  useLayoutEffect(() => {
    const result = getSubHeaderTitle();
    if (result.length > 0) {
      setBreadcrumbs(result[0].breadcrumbs);
      setSubHeaderTitle(result[0].name);
    } else {
